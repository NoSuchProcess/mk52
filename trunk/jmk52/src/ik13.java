...
class Tir2 {
    ...
    public short tact_exec2(short in_in) // Ускоренный вариант
    {
        Rg_In = in_in;
        if (micro_tact > 251) {
            micro_tact = 0;
    ...
    short Registr_M[] = new short[253];  //  short Registr_M[253];
    short Rg_In, Rg_Out;
    int micro_tact;
};
...
//    if Registr_S1[3] then result := result + 8;
    ...
    void write_ri(short i, short data) {
        Registr_R[i] = data;
    ...
    int get_mc() {
        ...
        k = (short) (micro_tact / 36);
        if (k < 3) {
            asp = (short) (0xFF & (cmd_130x[sk]));
        }
        if (k == 3) {
            asp = (short) (0xFF & (cmd_130x[sk] >> 8));
        }
        if (k == 4) {
            asp = (short) (0xFF & (cmd_130x[sk] >> 16));
        }
        md = (short) (0xFF & (cmd_130x[sk] >> 24));

        if ((k == 4) && (asp > 0x1F)) // стр. 115
        {
            if (micro_tact == 144) // if (micro_tact div 4) = 36 then
            {
                // write_ri(1, asp and $F); // Стр 145
                // write_ri(4, asp shr 4);

                write_ri((short) 37, (short) (asp & 0xF));
                write_ri((short) 40, (short) (asp >> 4));
        ...
        adr_mc = (short) (adr_mc & 0x3F);
        if (adr_mc > 59) {
            adr_mc = (short) ((adr_mc - 60) * 2);
            if (!(L > 0)) {
                adr_mc = (short) (adr_mc + 1);
            }
            adr_mc = (short) (adr_mc + 60);
        }

        return mc_130x[adr_mc];
    }

    boolean bool_(short i) {
        if (i > 0) {
            return true;
        } else {
            return false;
    ...
    short tact_exec(short in_in) // Ускоренный вариант
    {
        int four_1248, tact_0123;
        short x, y, z;
        short alfa, beta, gamma, sigma;
        short summa;
        short Signal_D, Signal_E, Signal_I;
        ...
        Signal_I = (short) (micro_tact / 4);	// 0..41
        Signal_D = (short) (micro_tact / 12);	// 0..13
        Signal_E = (short) ((micro_tact / 4) % 3);	// 0,1,2

        if (micro_tact == 0) {

            sk = (short) (0xff & (read_ri((short) 36) + (read_ri((short) 39) << 4)));
...
//    if ((micro_tact div 4) + 3) = (keyboard_x * 3) then // только E = 0 ? Ахинея ?
                if (((micro_tact / 12)) != (keyboard_x - 1)) {
                    if ((keyboard_y > 0)) {
                        // keyboard_y = K2 * 8 + K1
                        if (four_1248 == 1) {
                            Registr_S1 = (short) (Registr_S1 | keyboard_y);
                        }
                        // T := true;
                    }
                }
        ...
        if ((microcommand & 1) > 0) {
            alfa = (short) (alfa | Registr_R[Signal_I]);
        }
        if ((microcommand & 2) > 0) {
            alfa = (short) (alfa | Registr_M[Signal_I]);	//micro_tact
        }
        if ((microcommand & 4) > 0) {
            alfa = (short) (alfa | Registr_ST[Signal_I]);
        }
        if ((microcommand & 8) > 0) {
            alfa = (short) (alfa | ((~Registr_R[Signal_I]) & 0xF));
        }
        if ((microcommand & 16) > 0) //  alfa := alfa or ((($0A and four_1248) > 0) and (not L)); // 1,2,4,8 alfa := $0A * (not L); //  *
        {
            if (L == 0) {
                alfa = (short) (alfa | 0x0A);
            }
        }
        ...
        if ((microcommand & 32) > 0) {
            alfa = (short) (alfa | Registr_S);
        }
        if ((microcommand & 64) > 0) {
            alfa = (short) (alfa | 0x04);
        }

        if (((microcommand >> 7) & 16) > 0) {
            beta = (short) (beta | 0x01);
        }
        if (((microcommand >> 7) & 8) > 0) {
            beta = (short) (beta | 0x06);
        }
        if (((microcommand >> 7) & 4) > 0) {
            beta = (short) (beta | Registr_S1);
        }
        if (((microcommand >> 7) & 2) > 0) {
            beta = (short) (beta | ((~Registr_S) & 0xF));
        }
        if (((microcommand >> 7) & 1) > 0) {
            beta = (short) (beta | Registr_S);
        }
        ...
        /**
         * * {
         * if tact_0123 = 0 then if (Signal_D >= 0) and (Signal_D < 12) then if
         * L then segment_i8 := Signal_D;
         */
      ...
      /* if((micro_tact  /  4) == 36   )
        ...
        // summa := (short(alfa) and 1) + (short(beta) and 1) + (short(gamma) and 1);
        summa = (short) (alfa + beta + gamma);
        // if alfa then summa := summa + 1;
        // if beta then summa := summa + 1;
        // if gamma then summa := summa + 1;


        sigma = (short) (summa & 0xF);
        perenos = (short) (summa >> 4);
        ...
        if ((md == 0) || ((micro_tact / 4) >= 36)) {
            switch (((microcommand >> 15) & 7)) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    Registr_R[Signal_I] = (short) (Registr_R[Signal_I] | Registr_S | sigma);
                    break;
                case 5:
                    Registr_R[Signal_I] = (short) (Registr_S | sigma);
                    break;
                case 6:
                    Registr_R[Signal_I] = (short) (Registr_R[Signal_I] | Registr_S);
                    break;
                case 7:
                    Registr_R[Signal_I] = (short) (Registr_R[Signal_I] | sigma);
                    break;
            }
        ...
        switch (((microcommand >> 22) & 3)) {
            ...
            case 2:
                ...
                break;
            case 3:
                Registr_S = (short) (Registr_S1 | sigma);
                break;			//    Registr_S1 ?
        }
        switch (((microcommand >> 24) & 3)) {
            case 1:
            ...
            case 3:
                Registr_S1 = (short) (Registr_S1 | sigma);
        ...
        switch (((microcommand >> 26) & 3)) {
            ...
            case 3: {
                ...
                Registr_ST[(Signal_I + 0) % 42] = (short) (sigma | y);	// i+0
                Registr_ST[(Signal_I + 1) % 42] = (short) (x | z);	// i+1
                Registr_ST[(Signal_I + 2) % 42] = (short) (y | x);	// i+2
            }
            ;
            break;
        }
        ...
        /**
         * *{ рабочий вариант Rg_Out := Registr_M[0]; for( n := 0 to 166 do
         * Registr_M[n] := Registr_M[n + 1]; Registr_M[167] := Rg_In;
         */
        Rg_Out = (short) (0xF & Registr_M[Signal_I]);
    ...
    int mc_130x[] = new int[68]; //68
    int cmd_130x[] = new int[256]; //256
    short sp_130x[] = new short[1152]; // 1152
    short Registr_M[] = new short[42]; // 42
    short Registr_R[] = new short[42];
    short Registr_ST[] = new short[42];
    short Registr_S;
    short Registr_S1;
    short L, T, perenos;
    short H;
    ...
    short sk;
    short md;
    short keyboard_x, keyboard_y, segment_i8;
    short Rg_In, Rg_Out;
    short asp, adr_mc;
    boolean ind;
    boolean ind_multipoint[] = new boolean[14]; // 14
    short J[] = {0, 1, 2, 3, 4, 5, 3, 4, 5, 3, 4, 5, 3, 4, 5, 3,
...
class Tmk52 {
    ...
    void create() {
        ...
        for (i = 0; i <= 67; i++) {
            ...
            ik1302.mc_130x[i] = a.mc_1302[i];
            ik1303.mc_130x[i] = a.mc_1303[i];
            ik1306.mc_130x[i] = a.mc_1306[i];
        }

        for (i = 0; i <= 255; i++) {
            ik1302.cmd_130x[i] = a.cmd_1302[i];
            ik1303.cmd_130x[i] = a.cmd_1303[i];
            ik1306.cmd_130x[i] = a.cmd_1306[i];
        }

        for (i = 0; i <= 1151; i++) {
            ik1302.sp_130x[i] = a.sp_1302[i];
            ik1303.sp_130x[i] = a.sp_1303[i];
            ik1306.sp_130x[i] = a.sp_1306[i];
        }
    ...
    void pre_exec(short Rg_In) {
        ik1302.Registr_M[((ik1302.micro_tact / 4) + 41) % 42] = (short) Rg_In;
    }
